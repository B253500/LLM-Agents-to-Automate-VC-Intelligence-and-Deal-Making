...
def run_mermaid_style_crewai_orchestration(text, profile, file_path):
    ...
    chain_runner_agent = Agent(
        role="Orchestrator",
        goal="Schedule and dispatch memo‐generation sub-tasks.",
        backstory="Manages task dependencies and launches ready tasks in the crew.",
        verbose=False
    )

    # Define chain-only callbacks (ESG, BM, Exit, Follow-Up)
    from chains.esg_chain import run_esg_chain_with_text
    from chains.business_model_chain import run_business_model_chain_with_text
    from chains.exit_strategy_chain import run_exit_strategy_chain_with_text
    from chains.follow_up_chain import run_follow_up_chain_with_text
    ...
    def esg_chain_task(profile_dict):
        from core.schemas import StartupProfile
        p = StartupProfile(**profile_dict)
        return run_esg_chain_with_text(text, p).model_dump()

    def bm_chain_task(profile_dict):
        from core.schemas import StartupProfile
        p = StartupProfile(**profile_dict)
        return run_business_model_chain_with_text(text, p).model_dump()

    def exit_chain_task(profile_dict):
        from core.schemas import StartupProfile
        p = StartupProfile(**profile_dict)
        return run_exit_strategy_chain_with_text(text, p).model_dump()

    def followup_chain_task(profile_dict):
        from core.schemas import StartupProfile
        p = StartupProfile(**profile_dict)
        return run_follow_up_chain_with_text(text, p).model_dump()

    # Build all Task objects in dependency order
    deck_chain = Task(
    ...
    # Assemble all tasks into a single list
    tasks = [
    ...
    # Instantiate the manager LLM and create the Crew
    from langchain_openai import ChatOpenAI
    manager = ChatOpenAI(model="gpt-4o", temperature=0)

    from crewai import Crew, Process
    print("Available Process modes:", Process.__members__.keys())
    crew = Crew(
        tasks=tasks,
...
def main():
    ...
    for file_path in file_paths:
        ...
        profile = run_mermaid_style_crewai_orchestration(text, profile, file_path)          # Generate memo from enriched profile
